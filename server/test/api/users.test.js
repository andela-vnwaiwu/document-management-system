...
describe('User Suite', () => {
  ...
  before((done) => {
    ...
    db.Permission.bulkCreate(permissions, { returning: true })
      .then((createdPermissions) => {
        db.Role.bulkCreate(roles, {
          returning: true
        }).then((newRoles) => {
          db.RolePermission.bulkCreate(rolesPermission, { returning: true })
            .then(() => {
              ...
              factory.secondAdminrRoleId = adminRole.id;
  ...
  describe('Update user PUT: /api/users/:id', () => {
    ...
    it('should update the user\'s details for the admin or owner', (done) => {
      request
        .patch(`/api/users/${userDetails.id}`)
        .set('authorization', token)
        .send(updateDetails)
        .end((err, res) => {
          if (err) return done(err);
